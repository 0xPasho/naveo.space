...
- **Fase actual**: ejecución activa. Player de lecciones funcional, contenido inicial en MDX, evaluación con LLM-judge enchufada.
- **Ya en código**: Next.js 16 + i18n (`es` por defecto, `en`), Clerk, shadcn/ui, Prisma con pgvector, OpenRouter para LLM, módulos de dominio (`lessons`, `exercises`, `practice`, `daily-quest`, `dashboard`, `gamification`, `economy`, `srs`, `leaderboard`, `shop`, `rubric`, `tutor`, `cast`, `crew`, `catalog`, `content`, `llm`, `users`, `progress`, `blog`, `home`), contenido inicial bilingüe en `content/<locale>/{steps,daily,tracks.yaml}`, sistema de daily quests con frontmatter validado.
- **Pendiente**: pulido del knowledge bank con embeddings, expansión del catálogo de cursos, hardening de evaluaciones, estaciones por personaje (ver `10-estaciones.md`).
...
| 10 | [estaciones.md](./10-estaciones.md) | Estaciones por personaje: cada miembro del crew dueño de un dominio de conocimiento |
| 11 | [daily-quests.md](./11-daily-quests.md) | Daily quests: modelo, schema de frontmatter, kinds soportados, guía de autoría |
