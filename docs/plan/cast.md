...
### Orbit — operadora de misión (Track 4)

- **Rol pedagógico**: orquestadora. Descompone problemas en pasos, decide qué corre y en qué orden, mantiene loops que terminan. Aparece cuando el alumno diseña chains, routing y agent loops.
- **Personalidad**: metódica, paciente con los loops, dibuja el flujo en la pizarra antes de tocar código.
- **Aparece en**: Track 4 desde el primer step.
- **Visual**: silueta vertical con anillos concéntricos sutiles en el pecho que sugieren órbita / loop.

### Hex — analista de seguridad (Track 5)

- **Rol pedagógico**: red-teamer. Piensa como atacante y rompe el sistema antes de que lo haga un usuario hostil. Aparece cuando el alumno trabaja con inyección, fugas, tools destructivas, alucinación adversarial.
- **Personalidad**: seca, curiosa, sin drama. No es villano — es la profesional que firma "no encontré nada" cuando el sistema aguanta.
- **Aparece en**: Track 5 desde el primer step. Atlas sigue firmando el cierre.
- **Visual**: cuerpo hexagonal redondeado con tres marcas sutiles tipo `✦` que sugieren "vulnerabilidad encontrada".

### Personajes futuros

Se inventan según se necesiten — **un personaje por arquetipo de habilidad**, no por capricho. Antes de agregar uno, hay que poder describir su habilidad en una sola línea.
