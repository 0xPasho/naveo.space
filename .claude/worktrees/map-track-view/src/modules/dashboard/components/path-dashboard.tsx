...
export async function PathDashboard({ dashboard }: Props) {
  ...
  const continueHref = continueAt
    ...
    : "/tracks"
  // "Open the Map" jumps to the current course's full Duolingo-style map.
  // Falls back to the catalog when the user has no in-flight course.
  const mapHref = continueAt
    ? `/tracks/${continueAt.next.trackSlug}/${continueAt.next.courseSlug}`
    : "/tracks"
          ...
          >
            <div className="card">
              <div className="card-h">
                ...
                <Link href={mapHref} className="more">
