...
export default async function ClearedPage({ params }: Props) {
  ...
  const [track, course, steps, detail] = await Promise.all([
    getTrack(trackSlug, contentLocale),
    getCourse(courseSlug, contentLocale),
    listSteps(courseSlug, contentLocale),
    getCourseDetail(userId, trackSlug, contentLocale),
  ])
  ...
  const allTracks = await listTracks(contentLocale)
