...
export async function BridgeDashboard() {
  ...
  if (clerkUser) {
    ...
    const [stats, snap, week] = await Promise.all([
  ...
  // XP + streak are live; gems/hearts remain placeholder.
  const telemetry: Array<{ label: string; value: string; color: string }> = [
    ...
    { label: t("telemetry.gems"), value: "38", color: "var(--plasma-violet)" },
    { label: t("telemetry.hearts"), value: "4 / 5", color: "var(--hazard-red)" },
    {
