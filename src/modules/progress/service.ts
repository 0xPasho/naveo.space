...
export async function markStepCompleted(args: {
  ...
  // Daily-progress signal — narrative completions count toward the streak,
  // not just exercise attempts. Best-effort; never blocks the response.
  await recordActivity(userId, "step-attempt").catch((err) => {
    console.error("[streak] markStepCompleted → recordActivity failed", err)
  })
