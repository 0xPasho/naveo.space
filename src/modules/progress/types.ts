...
export type StepProgress = {
  ...
  attempts: number
  hintsUsed: number
  // Frozen at first completion. True iff the user passed this step on attempt
  // #1. Stays true even if attempts climbs later (re-runs after pass don't
  // demote it). Used by XP recalculators so the cleared/debrief screen keeps
  // the 1.5x first-try bonus the user actually earned.
  firstTry: boolean
